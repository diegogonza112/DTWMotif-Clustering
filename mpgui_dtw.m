classdef mpgui_dtw < handle
    ...
    methods
        ...
        function plotMotif(gui, axhandle, meas, motifidx, neighboridx,warp_path)
            ...
            if(~exist('warp_path','var') || isempty(warp_path)) % Just plot neigh - no warp path
                ...
                neigh_warp = gui.data(neighboridx+warp_path-1); %DK kludge to enable warping path to be displayed
                neigh = (neigh_warp - mean(neigh_warp)) ./std(neigh_warp, 1);
            end
        ...
        function plotMotifdtw(gui, motifidx, neighboridx,warp_path)
            if(~exist('warp_path','var') || isempty(warp_path)) % Just plot neigh - no warp path
                gui.plotMotif(gui.motifAxdtw, 'dynamic time warping', motifidx, neighboridx,warp_path);
            else
                gui.plotMotif(gui.motifAxdtw, 'dynamic time warping (*)', motifidx, neighboridx,warp_path);
            end
            %hold(axhandle, 'off');
